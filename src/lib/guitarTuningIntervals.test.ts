...
test('equivalent fretboard locations count as the same absolute pitch', () => {
  assert.equal(isSameGuitarPitch(3, 4, 4, 9), true);
  assert.equal(isSameGuitarPitch(3, 4, 4, 8), false);
  assert.equal(isSameGuitarPitch(3, 4, 3, 16), false);
});
